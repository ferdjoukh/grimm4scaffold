...
public class OclPredicate {
	...
	// Noms des noeuds XML contenant une contrainte ou un prédicat
	...
	public Element getConstraintXCSP(String ... parameters) {
		if (parameters.length != _arity) {
			System.err.println("Le nombre de paramètres données (" + parameters.length + ") à " + _name + 
					" ne correspond pas au nombre attendu (" + _arity + "). Paramètres donnés : " + Arrays.toString(parameters));
			return null;
		} else {
