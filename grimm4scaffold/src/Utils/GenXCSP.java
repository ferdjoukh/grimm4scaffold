...
public class GenXCSP {
	...
	public GenXCSP(String modelFile,String racine,ModelReader re, ConfigFileReader cfr, int sym)
	{
		...
		this.FeatureBound=cfr.getRefsBound();
		
		this.racine=racine;
		//System.out.println("FeaturesB="+this.FeatureBound);
		this.RefsBound= cfr.getRefsBound()*2;
...
*/
	...
	{
		int i=1;
		int globalarityGcc=0;
    	int globalvalsArityGcc=0;
    	String globalvarsGcc="";
    	String globalvalsGcc="";
		for(EClass c: lesClasses)
		{
			int refi=0;
			...
			{	
    		    ...
    		    for (EReference ref: r.getAllReferencesFromClasswithOpposite(c))
    			{
    		    	...
    		    	if (!ref.getEReferenceType().isAbstract())
    				{
                    	...
                    	////////////////////////////////
                    	//
                    	//  	Create non-equal constraints, and not these Edges
                    	//
                    	////////////////////////////////
                    	if(ref.getName().equals("EVin"))
                    	{
                    		GenConstNeq("Id_"+c.getName()+"_"+j+"_"+ref.getName()+"_"+z, "Id_"+c.getName()+"_"+j+"_EVout_"+z);
                    		notTheseEdgesConstraint("Id_"+c.getName()+"_"+j+"_"+ref.getName()+"_"+z, "Id_"+c.getName()+"_"+j+"_EVout_"+z);
                    		notTheseEdgesConstraint("Id_"+c.getName()+"_"+j+"_EVout_"+z,"Id_"+c.getName()+"_"+j+"_"+ref.getName()+"_"+z);
                    	}
						...
						//Les vars et arityVars de la Gcc
							...
							for(int j=domaineSum(r.getClassIndex(c)-1)+1;j<=domaineSum(r.getClassIndex(c));j++)   //10= size(c);
							{
								for(int z=1;z<=zz;z++)
								{
									...
									globalarityGcc++;
									globalvarsGcc+= "Id_"+c.getName()+"_"+j+"_"+ref.getName()+"_"+z+" ";	
								}
							}
					

					    Uniform kimjongUn= new Uniform(RefsBound, RefsBound);
						ArrayList<Integer> upperBounds= null;
					
						Generator gene= cfr.getDegreedDistrib();
						
						if(gene instanceof LoiExponentielle)
						{
							upperBounds= ((LoiExponentielle) gene).generate(globalvalsArityGcc+1);
							
						}
						else if (gene instanceof LoiNormale)
						{
							upperBounds= ((LoiNormale) gene).generate(globalvalsArityGcc+1);
						}
						else if (gene instanceof Uniform)
						{
							upperBounds= ((Uniform) gene).generate(globalvalsArityGcc+1);
						}
						else
						{
							upperBounds = kimjongUn.generate(globalvalsArityGcc+1);
						}
						
						createGcc2(ref.getName()+"_"+ref.getEOpposite().getName(),arityGcc,valsArityGcc,varsGcc,valsGcc,0,upperBounds);	
					}
				}
			}
		...
		///////////////////////////////////////////////////////////////
		//Ajouter une contrainte Gcc globale des degres 
		//////////////////////////////////////////////////////////////
		...
		for(int k=JSLB;k<=JSUB;k++)
			jokerString+= k+" ";
		
		int nbvJ= JSUB-JSLB+1;
		
		globalvalsGcc+= jokerString;
		globalvalsArityGcc+= nbvJ;
		
		LoiExponentielle loiExpo= new LoiExponentielle(2.0);
		...
		{
			upperBounds = loiExpo.generate(globalvalsArityGcc+1);
		}
		
		
		/*System.out.println("Gcc( \n"
							+ "["+ globalvarsGcc +"] ("+ globalarityGcc +") \n"
							+ "["+ globalvalsGcc +"] ("+ globalvalsArityGcc +") \n"	
							+ "[0] \n"
							+ "[upper] \n"
							+ ")");
		*/
		
		//loiExpo.prettyPrint(upperBounds);
		
	//	createGcc2("GlobalDegre", globalarityGcc, globalvalsArityGcc, globalvarsGcc, globalvalsGcc, 1, upperBounds);
	//	createGcc3("GlobalDegre", globalarityGcc, globalvalsArityGcc, globalvarsGcc, globalvalsGcc, upperBounds, upperBounds);
	...
	*/	
	...
	public void createGcc2(String nom,int arity,int valsarity,String vars,String vals, int lower, ArrayList<Integer> upper)
	{
		Element cons=new Element("constraint");
		nbCons++;
	...
	////////////////////////////////////////////////
	//////////////////
	/////////////////   Gen Non-equal predicate and constraint
	////////////////
	public void GenPredNeq()
	{
		//La relation d'implication
				Element pre1= new Element("predicate");
				nbPre++;
				...
				Element exp= new Element("expression");
				Element fct= new Element("functional");
				fct.setText("ne(v1,v2)");
				exp.addContent(fct);
				
				pre1.addContent(par1);
				pre1.addContent(exp);
				Attribute refe1= new Attribute("name", "nonEqual");
				pre1.setAttribute(refe1);
				predicates.addContent(pre1);
	...
	public void GenConstNeq(String v1,String v2)
	{
		Element cons = new Element("constraint");
		nbCons++;
		...
		Attribute name= new Attribute("name", "ConsNeq"+v1+v2);
		cons.setAttribute(name);
		Attribute Arity= new Attribute("arity", "2");
		cons.setAttribute(Arity);
		Attribute Scope= new Attribute("scope", v1+" "+v2);
		cons.setAttribute(Scope);
		Attribute refe= new Attribute("reference", "nonEqual");
		cons.setAttribute(refe);
	...
	///////////////////////////
	//////////////////
	////////////  Generate "Not All These Edges" relation
	//////
	///
	public void NotTheseEdgesRelation()
	{
		Element rel = new Element("relation");
		nbRel++;
		
		
		Attribute name= new Attribute("name", "NotTheseEdges");
		rel.setAttribute(name);
		Attribute Arity= new Attribute("arity", "2");
		rel.setAttribute(Arity);
		
		int i= domaineSumMin(r.getClassIndex("Vertex"));
		int j= domaineSum(r.getClassIndex("Vertex"));
		
		Attribute Scope= new Attribute("nbTuples", ""+((j-i+1)/2));
		rel.setAttribute(Scope);
		
		Attribute refe= new Attribute("semantics", "conflicts");
		rel.setAttribute(refe);
		...
		rel.setText(texte+""+(j-1)+" "+j+ " ");
		
		relations.addContent(rel);
	}
	
	public void notTheseEdgesConstraint(String a,String b)
	{
		Element cons = new Element("constraint");
		nbCons++;
	
		Attribute name= new Attribute("name", "notThisEdgeCons"+a+b);
		cons.setAttribute(name);
		Attribute Arity= new Attribute("arity", "2");
		cons.setAttribute(Arity);
		Attribute Scope= new Attribute("scope", a+" "+b);
		cons.setAttribute(Scope);
		Attribute refe= new Attribute("reference", "NotTheseEdges");
		cons.setAttribute(refe);
	...
	public Document GenerateXCSP(String file)
	{
		...
		GenFeaturesDomains(FeatureBound+10);//FeatureBound);
